      ...
      (cond ((plusp transmit)
             (lambda (intersection ray)
               (if (weakp ray)
                   ...
                   (multiple-value-bind (reflected refracted)
                       (spawn-rays intersection ray specular transmit ior)
            ...
            ((plusp specular)
             (lambda (intersection ray)
               (if (weakp ray)
                   black
                   (raytrace (reflected-ray intersection ray specular) scene))))
            (t
...
(defmethod shader-function ((shader flat) scene)
  (let ((ambient-color (hadamard-product
                        ...
                        (color-of shader))))
    (lambda (intersection ray)
      (declare (ignore intersection ray))
      ambient-color)))
...
(defmethod shader-function ((shader solid) scene)
  (let* ((color (color-of shader))
	...
	(let ((color ambient-color)
	      ...
	      (normal (intersection-normal intersection)))
...
(defmethod shader-function ((shader checker) scene)
  (let ((odd (compile-shader (odd-of shader) scene))
    ...
    (lambda (intersection ray)
      (let ((point (intersection-point intersection)))
  ...
  (let* ((functions (mapcar (lambda (shader) (compile-shader shader scene))
    ...
    (lambda (intersection ray)
      (vector-div (reduce (lambda (c f)
			    (vector-add c (funcall f intersection ray)))
			  functions
			  :initial-value black)
		  count))))
